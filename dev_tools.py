...
def start_release(version):
    ...
    if Path("docs/conf.py").exists():
        ...
        updated_content = re.sub(
            r"release = ['\"][^'\"]+['\"]", f"release = '{version}'", updated_content
        )
    ...
    print("💡 Next steps:")
    print("   1. Test thoroughly on this branch")
    print(f"   2. Push branch: git push -u origin {branch_name}")
    print("   3. Run: python dev_tools.py merge-to-main")
...
def test_package_locally():
    ...
    if wheel_files:
        ...
        import_cmd = ("import threepanewindows; "
                      "print('Version:', threepanewindows.__version__)")
        run_command([python_exe, "-c", import_cmd])
    else:
...
def run_lint():
    ...
    checks = [
        ...
        (
            ["python", "-m", "mypy", "threepanewindows/", "--ignore-missing-imports"],
            "MyPy type checking",
        ),
...
def run_examples():
    ...
    stdout, stderr = run_command(
        ["python", "-m", "threepanewindows.examples"], check=False
    )
...
def setup_dev_env():
    ...
    commands = [
        ...
        (
            ...
            "Installing package in dev mode",
        ),
...
def show_help():
    ...
    print("=" * 60)
    print()
    print("This script provides helpful commands for the "
          "ThreePaneWindows development workflow.")
    print()
...
"""
    create_summary(
        f"Feature Branch Created: {feature_name}", summary_content, "FEATURE"
    )
...
def handle_merge_to_main_command():
    """Handle the merge-to-main command."""
    success = merge_to_main()

    if success:
        current_branch = get_current_branch()
        version = (
            current_branch.replace("release/v", "")
...
def handle_test_command():
    ...
    # Create summary
    summary_content = """Performed local package testing
...
def handle_lint_command():
    ...
    next_steps = (
        "Fix any issues shown above and re-run checks"
        if not success
        else "All checks passed! Code is ready for commit."
    )
...
def handle_docs_command():
    ...
    next_steps = (
        "Open docs/_build/html/index.html to view documentation"
        if success
        else "Check error messages and fix documentation issues"
    )
...
def handle_clean_command():
    ...
    # Create summary
    summary_content = """Cleaned build artifacts and cache files
...
def handle_pytest_command():
    """Handle the pytest command."""
    success = run_pytest()
    ...
    next_steps = (
        ...
        if success
        else "Fix failing tests and re-run pytest"
    )
...
def handle_examples_command():
    ...
    next_steps = (
        ...
        if success
        else "Check error messages and fix example issues"
    )
...
def handle_setup_command():
    ...
    next_steps = (
        "Development environment ready! Try running examples or tests."
        if success
        else "Fix setup issues and re-run setup command"
    )
...
def main():
    ...
    command_handlers = {
        "feature": handle_feature_command,
        "release": handle_release_command,
        "merge-to-main": handle_merge_to_main_command,
        "test": handle_test_command,
        "lint": handle_lint_command,
        "docs": handle_docs_command,
        "clean": handle_clean_command,
        "pytest": handle_pytest_command,
        "examples": handle_examples_command,
    ...
    if command in command_handlers:
        ...
        available_commands = ("setup, feature, release, merge-to-main, test, "
                              "pytest, lint, docs, examples, clean, status, "
                              "help, summary")
