...
class TestSecurityFeatures(unittest.TestCase):
    ...
    def test_exception_handling_patterns(self):
        ...
        self.assertIn("# nosec B110", source, "Should have nosec comments for security exceptions")
        
        # Check that we're using specific exceptions
        self.assertIn("tk.TclError", source, "Should use specific TclError exceptions")
        self.assertIn("AttributeError", source, "Should use specific AttributeError exceptions")
    ...
    def test_subprocess_security_patterns(self):
        ...
        try:
            ...
            self.assertIn("# nosec B603", source, "Should have nosec comments for subprocess calls")
            self.assertIn("check=False", source, "Should use check=False for safer subprocess calls")
        ...
        try:
            ...
            # Check for security patterns
            self.assertIn("# nosec B603", source, "Should have nosec comments for subprocess calls")
            self.assertIn("# nosec B404", source, "Should have nosec comments for subprocess imports")
            self.assertIn("shutil.which", source, "Should use shutil.which for path validation")
    ...
    def test_platform_security_imports(self):
        ...
        try:
            from threepanewindows.utils import linux, macos, windows
    ...
    def test_bandit_security_compliance(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                f"Bandit should pass or only have nosec warnings. Output: {result.stdout} {result.stderr}"
            )
