...
import math
from .themes import ThemeManager, get_theme_manager, Theme
...
class PaneConfig:
    ...
    closable: bool = False
    fixed_width: Optional[int] = None  # If set, pane will stick to this width and not be resizable


class DragHandle(tk.Frame):
    """Professional drag handle for detaching panes."""
    
    def __init__(self, parent, pane_side: str, on_detach: Callable, theme_manager: ThemeManager, **kwargs):
...
class PaneHeader(tk.Frame):
    ...
    def __init__(self, parent, config: PaneConfig, pane_side: str, 
                 on_detach: Callable, on_close: Optional[Callable], 
                 theme_manager: ThemeManager, **kwargs):
        super().__init__(parent, **kwargs)
        self.config = config
        self.pane_side = pane_side
        self.on_detach = on_detach
    ...
    def _setup_ui(self):
        ...
        if self.config.closable and self.on_close:
            close_btn = self._create_control_button(right_frame, "✕", self.on_close, "Close")
            close_btn.pack(side="right", padx=2)
        
        # Detach button
        if self.config.detachable:
            detach_btn = self._create_control_button(right_frame, "⧉", self.on_detach, "Detach")
    ...
    def _create_control_button(self, parent, text: str, command: Callable, tooltip: str) -> tk.Button:
        ...
        def on_leave(e):
            btn.configure(bg=theme.colors.panel_header_bg, fg=theme.colors.secondary_text)
...
class DetachedWindow(tk.Toplevel):
    ...
    def __init__(self, parent, pane_side: str, config: PaneConfig, 
                 content_builder: Callable, on_reattach: Callable,
                 theme_manager: ThemeManager, **kwargs):
        super().__init__(parent, **kwargs)
        self.pane_side = pane_side
        self.config = config
    ...
    def _setup_window(self):
        ...
        if self.config.icon:
            try:
                # You could set a window icon here if you have icon files
                pass
            except:
                pass
    ...
    def _setup_ui(self):
        ...
        if self.config.title:
            title_label = tk.Label(
                header_frame,
                text=f"{self.config.icon} {self.config.title}" if self.config.icon else self.config.title,
                bg=theme.colors.panel_header_bg,
                fg=theme.colors.panel_header_fg,
                font=(theme.typography.font_family, theme.typography.font_size_normal, "bold")
            )
...
class EnhancedDockableThreePaneWindow(tk.Frame):
    ...
    def __init__(self, master=None, 
        ...
        self.center_config = center_config or PaneConfig(title="Main Content", icon="📝", detachable=False)
    ...
    def _create_widgets(self):
        ...
        # Create paned window
        self.paned = ttk.PanedWindow(self, orient=tk.HORIZONTAL, style="Themed.TPanedwindow")
    ...
    def _create_left_pane(self):
        ...
        # Content frame
        content_frame = tk.Frame(container, bg=self.theme_manager.get_current_theme().colors.panel_content_bg)
        ...
        if self.left_config.fixed_width is not None:
            self.paned.after_idle(lambda: self._configure_fixed_pane_width("left", self.left_config.fixed_width))
    ...
    def _create_center_pane(self):
        ...
        if self.center_config.title:
            header = PaneHeader(
                ...
                self.center_config,
                "center",
                lambda: self._detach_pane("center") if self.center_config.detachable else None,
                None,
                self.theme_manager
            )
            header.pack(fill="x", padx=0, pady=0)
    ...
    def _create_right_pane(self):
        ...
        # Configure pane width constraints if fixed width is set
        if self.right_config.fixed_width is not None:
            self.paned.after_idle(lambda: self._configure_fixed_pane_width("right", self.right_config.fixed_width))
    ...
    def _configure_fixed_pane_width(self, pane_side: str, fixed_width: int):
        ...
        if pane_index is not None:
            ...
            try:
                ...
                self.paned.paneconfig(pane_index, minsize=fixed_width, width=fixed_width)
    ...
    def get_pane_frame(self, pane_side: str) -> Optional[tk.Frame]:
        ...
        if pane_side in self.detached_windows:
            ...
            for child in self.pane_frames[pane_side].winfo_children():
                if isinstance(child, tk.Frame) and child != self.pane_headers.get(pane_side):
                    return child
        return None
